...
describe("ICO Contract", function () {
    ...
    describe("Deployment", function () {
        it("Should deploy with correct initial values", async function () {
            ...
            expect(await ico.maxTokensToSell()).to.equal(ethers.parseEther("10000"));
        ...
        it("Should revert with zero maxTokensToSell", async function () {
            const { mockToken, ethPriceFeed, usdtPriceFeed, usdcPriceFeed, btcPriceFeed, scrPriceFeed, usdtToken, usdcToken, wbtcToken, scrToken } = await loadFixture(deployICOFixture);
            const ICO = await ethers.getContractFactory("ICO");
            
            await expect(
                ICO.deploy(
                    await mockToken.getAddress(),
                    0, // Invalid maxTokensToSell
                    user1.address, // treasury wallet
                    await ethPriceFeed.getAddress(),
                    await usdtPriceFeed.getAddress(),
                    await usdcPriceFeed.getAddress(),
    ...
    describe("Access Control", function () {
        it("Should set up admin role correctly on deployment", async function () {
            const { ico, owner } = await loadFixture(deployICOFixture);
            
            // Owner should have admin role by default
            expect(await ico.hasAdminRole(owner.address)).to.be.true;
            
            // Check that ADMIN_ROLE constant is set correctly
            const adminRole = await ico.ADMIN_ROLE();
        ...
        it("Should allow admin to grant admin role to others", async function () {
            ...
            // Initially user1 should not have admin role
            expect(await ico.hasAdminRole(user1.address)).to.be.false;
            
            // Owner grants admin role to user1
            await ico.connect(owner).grantAdminRole(user1.address);
            
            // Now user1 should have admin role
            expect(await ico.hasAdminRole(user1.address)).to.be.true;
        });

        it("Should allow admin to revoke admin role from others", async function () {
            const { ico, owner, user1 } = await loadFixture(deployICOFixture);
            
            // First grant admin role to user1
            await ico.connect(owner).grantAdminRole(user1.address);
            expect(await ico.hasAdminRole(user1.address)).to.be.true;
            
            // Now revoke it
            await ico.connect(owner).revokeAdminRole(user1.address);
            
            // user1 should no longer have admin role
            expect(await ico.hasAdminRole(user1.address)).to.be.false;
        });

        it("Should allow admin to revoke their own admin role", async function () {
            const { ico, owner, user1 } = await loadFixture(deployICOFixture);
            
            // Grant admin role to user1
            await ico.connect(owner).grantAdminRole(user1.address);
            
            // user1 revokes their own admin role
            await ico.connect(user1).revokeAdminRole(user1.address);
        ...
        it("Should prevent non-admin from granting admin role", async function () {
            ...
            await expect(
                ico.connect(user1).grantAdminRole(user2.address)
            ).to.be.revertedWithCustomError(ico, "AccessControlUnauthorizedAccount");
        });

        it("Should prevent non-admin from revoking admin role", async function () {
            const { ico, owner, user1, user2 } = await loadFixture(deployICOFixture);
            
            // Grant admin role to user1
            await ico.connect(owner).grantAdminRole(user1.address);
            
            // user2 (non-admin) tries to revoke user1's admin role
            await expect(
                ico.connect(user2).revokeAdminRole(user1.address)
            ).to.be.revertedWithCustomError(ico, "AccessControlUnauthorizedAccount");
        });

        it("Should allow multiple admins to coexist", async function () {
            const { ico, owner, user1, user2 } = await loadFixture(deployICOFixture);
            
            // Grant admin role to both user1 and user2
            await ico.connect(owner).grantAdminRole(user1.address);
            await ico.connect(owner).grantAdminRole(user2.address);
            
            // Both should have admin role
            expect(await ico.hasAdminRole(user1.address)).to.be.true;
            expect(await ico.hasAdminRole(user2.address)).to.be.true;
            
            // user1 should be able to revoke user2's admin role
            await ico.connect(user1).revokeAdminRole(user2.address);
            expect(await ico.hasAdminRole(user2.address)).to.be.false;
            
            // user1 should still have admin role
            expect(await ico.hasAdminRole(user1.address)).to.be.true;
        });

        it("Should handle admin role management events", async function () {
            const { ico, owner, user1 } = await loadFixture(deployICOFixture);
            
            // Test granting admin role event
            await expect(ico.connect(owner).grantAdminRole(user1.address))
                .to.emit(ico, "RoleGranted")
                .withArgs(await ico.ADMIN_ROLE(), user1.address, owner.address);
            
            // Test revoking admin role event
            await expect(ico.connect(owner).revokeAdminRole(user1.address))
                .to.emit(ico, "RoleRevoked")
                .withArgs(await ico.ADMIN_ROLE(), user1.address, owner.address);
        });
    });
    ...
    describe("Admin Functions", function () {
        ...
        it("Should reject slippage protection update from non-admin", async function () {
            const { ico, user1 } = await loadFixture(deployICOFixture);
        ...
        it("Should allow admin role users to update slippage protection", async function () {
            const { ico, owner, user1 } = await loadFixture(deployICOFixture);
            ...
            await expect(ico.connect(user1).updateSlippageProtection(newSlippage))
                .to.emit(ico, "SlippageProtectionUpdated")
                .withArgs(newSlippage);
    ...
    describe("Token Limit Functionality", function () {
        it("Should enforce token limit on purchases", async function () {
            const { ico, mockToken, user1, user2 } = await loadFixture(deployICOFixture);
            
            // Calculate how much ETH is needed to reach the token limit
            const maxTokens = await ico.maxTokensToSell();
            const ethPrice = MOCK_PRICES.ETH;
            const tokenRate = TOKEN_RATE_USD;
            
            // Calculate USD value needed: maxTokens * tokenRate
            const usdValueNeeded = (maxTokens * tokenRate) / ethers.parseEther("1");
            
            // Calculate ETH amount needed: usdValueNeeded / ethPrice
            const ethAmountNeeded = (usdValueNeeded * ethers.parseUnits("1", 8)) / ethPrice;
            
            // Make a purchase that would exceed the limit
            const excessEthAmount = ethAmountNeeded + ethers.parseEther("1"); // Add 1 ETH more
            
            await expect(
                ico.connect(user1).buyTokensWithEth(user1.address, 0, { value: excessEthAmount })
            ).to.be.revertedWithCustomError(ico, "TokenLimitExceeded");
        });

        it("Should allow purchases up to the token limit", async function () {
            const { ico, mockToken, user1 } = await loadFixture(deployICOFixture);
            
            // Calculate how much ETH is needed to reach exactly the token limit
            const maxTokens = await ico.maxTokensToSell();
            const ethPrice = MOCK_PRICES.ETH;
            const tokenRate = TOKEN_RATE_USD;
            ...
            await ico.connect(user1).buyTokensWithEth(user1.address, 0, { value: ethAmountNeeded });
            
            const finalBalance = await mockToken.balanceOf(user1.address);
            expect(finalBalance - initialBalance).to.equal(maxTokens);
        });

        it("Should emit TokenLimitReached event when limit is exceeded", async function () {
            const { ico, mockToken, user1 } = await loadFixture(deployICOFixture);
            
            // First, fill up the ICO to the limit
            const maxTokens = await ico.maxTokensToSell();
            const ethPrice = MOCK_PRICES.ETH;
            const tokenRate = TOKEN_RATE_USD;
            ...
            // Now try to buy more tokens - should emit TokenLimitReached event
            const smallAmount = ethers.parseEther("0.001");
            
            await expect(
                ico.connect(user1).buyTokensWithEth(user1.address, 0, { value: smallAmount })
            ).to.be.revertedWithCustomError(ico, "TokenLimitExceeded");
        });

        it("Should track remaining tokens correctly", async function () {
            const { ico, mockToken, user1 } = await loadFixture(deployICOFixture);
            
            const maxTokens = await ico.maxTokensToSell();
            const initialRemaining = await ico.getRemainingTokens();
            expect(initialRemaining).to.equal(maxTokens);
            ...
            const remainingAfterPurchase = await ico.getRemainingTokens();
            expect(remainingAfterPurchase).to.equal(maxTokens - expectedTokens);
        });

        it("Should return correct ICO active status", async function () {
            const { ico, mockToken, user1 } = await loadFixture(deployICOFixture);
            
            // Initially should be active
            expect(await ico.isIcoActive()).to.be.true;
            
            // Fill up the ICO to the limit
            const maxTokens = await ico.maxTokensToSell();
            const ethPrice = MOCK_PRICES.ETH;
            const tokenRate = TOKEN_RATE_USD;
        ...
        it("Should handle partial token limit scenarios", async function () {
            const { ico, mockToken, user1, user2 } = await loadFixture(deployICOFixture);
            
            // Make a purchase that uses most of the limit
            const maxTokens = await ico.maxTokensToSell();
            const ethPrice = MOCK_PRICES.ETH;
            const tokenRate = TOKEN_RATE_USD;
            ...
            // Use 90% of the limit
            const partialAmount = (ethAmountNeeded * 9000n) / 10000n;
            await ico.connect(user1).buyTokensWithEth(user1.address, 0, { value: partialAmount });
            
            // Should still be active
            expect(await ico.isIcoActive()).to.be.true;
            
            // Try to use the remaining 20% - should fail
            const remainingAmount = (ethAmountNeeded * 2000n) / 10000n;
            await expect(
                ico.connect(user2).buyTokensWithEth(user2.address, 0, { value: remainingAmount })
            ).to.be.revertedWithCustomError(ico, "TokenLimitExceeded");
        });
    });
