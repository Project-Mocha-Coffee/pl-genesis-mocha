...
describe("MochaTreeToken", async function () {
  let DLT;
  let DLTReceiver;
  let DLTNonReceiver;
  ...
  before("Deployments", async function () {
    ...
    // ------------------------------------------------------------------
    const DLTNonReceiverFactory =
      await ethers.getContractFactory("DLTNonReceiver");
  ...
  beforeEach("Restart Deployment DLT at each test use case", async function () {
    // ------------------------------------------------------------------
    const DLTFactory = await ethers.getContractFactory("MochaTreeToken");
    DLT = await DLTFactory.deploy("1.0");
  ...
  describe("Should reflect balances", async function () {
    ...
    it("Should correctly return selector when calling onDLTReceived in DLTReceiver", async function () {
      const ZeroAddress = ethers.ZeroAddress;

      const result = await DLTReceiver.onDLTReceived.staticCall(
        ZeroAddress,
        ZeroAddress,
        1,
        1,
      ...
      expect(result).to.equal("0x19a36752");
    });

    it("Should correctly return selector when calling onDLTBatchReceived in DLTReceiver", async function () {
      const ZeroAddress = ethers.ZeroAddress;

      const result = await DLTReceiver.onDLTBatchReceived.staticCall(
        ZeroAddress,
        ZeroAddress,
        [1],
        [1],
      ...
      expect(result).to.equal("0xc20f1e19");
    });
  ...
  describe("TreeManager", function () {
    it("Should set tree manager address correctly", async function () {
      const user1Address = await user1.getAddress();
      await DLT.setTreeManagerAddress(user1Address);

      expect(await DLT.TreeManager()).to.equal(user1Address);
    });

    it("Should revert when non-owner tries to set tree manager", async function () {
      // The error message changed because Ownable in more recent versions uses custom errors
      await expect(
        DLT.connect(user1).setTreeManagerAddress(await user1.getAddress())
      )
        .to.be.revertedWithCustomError(DLT, "OwnableUnauthorizedAccount")
  ...
  describe("Admin functions", function () {
    ...
    it("Should allow TreeManager to call admin functions", async function () {
      await DLT.setTreeManagerAddress(user1.getAddress());

      await expect(
        DLT.connect(user1).thirdPartyApprove(
          owner.getAddress(),
          user1.getAddress(),
          1,
          1,
    ...
    it("Should revert when non-admin calls restricted functions", async function () {
      const nonAdmin = (await ethers.getSigners())[2];

      await expect(
        DLT.connect(nonAdmin).thirdPartyApprove(
          owner.getAddress(),
          user1.getAddress(),
          1,
          1,
      ...
      ).to.be.revertedWith("Only Admin is authorized");
    });
  });

  describe("ThirdPartyApprove", function () {
    it("Should allow admin to approve on behalf of others", async function () {
      await DLT.thirdPartyApprove(
        owner.getAddress(),
        user1.getAddress(),
        1,
        1,
    ...
    it("Should not allow approval to same owner", async function () {
      await expect(
        DLT.thirdPartyApprove(
          owner.getAddress(),
          owner.getAddress(),
  ...
  describe("Transfer", function () {
    beforeEach(async function () {
      // Make sure owner has tokens first
      await DLT.mint(owner.getAddress(), 1, 1, ethers.parseEther("1000"));
    });
    ...
    it("Should transfer tokens correctly", async function () {
      ...
      expect(await DLT.subBalanceOf(user1Address, 1, 1)).to.equal(
        ethers.parseEther("100")
      );
    });
    ...
    it("Should revert on insufficient balance", async function () {
      ...
      const currentBalance = await DLT.subBalanceOf(ownerAddress, 1, 1);
      const transferAmount = currentBalance + BigInt(ethers.parseEther("1000")); // Try to transfer more than balance

      await expect(
        DLT.transfer(ownerAddress, user1Address, 1, 1, transferAmount)
      ).to.be.revertedWith("DLT: insufficient balance for transfer");
    });
  });
  ...
  describe("Allow", function () {
    ...
    it("Should revert allow from zero address", async function () {
      await expect(
        DLT.allow(
          ethers.ZeroAddress,
          user1.getAddress(),
