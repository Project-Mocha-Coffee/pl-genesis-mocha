...
import InvestTreesDialog from "@/components/@shared-components/InvestTreesDialog";
...
export default function Dashboard() {
  ...
  const [selectedTrees, setSelectedTrees] = useState(1);
  ...
  const totalSupplyContracts = farmConfigsData
    ? farmConfigsData.map((result) => ({
      address: result.status === "success" ? result.result.shareTokenAddress : undefined,
      abi: [
        {
          "inputs": [],
          "name": "totalSupply",
  ...
  const { data: totalSupplyData, isLoading: isLoadingTotalSupplies } = useReadContracts({
    contracts: totalSupplyContracts,
    query: { enabled: !!farmConfigsData }
  });
  ...
  const farms = farmConfigsData
    ? farmConfigsData.map((result, index) => ({
      ...
      totalSupply: totalSupplyData?.[index]?.status === "success"
        ? totalSupplyData[index].result
        : BigInt(0),
  ...
  const firstAvailableFarm = farms.find(
    farm =>
      farm.config?.active &&
      farm.config?.treeCount > 0 &&
      farm.totalSupply > 0
  );

  console.log("First available farm:", firstAvailableFarm);
  console.log(formatUnits(BigInt(142673433333333325000), 18))
  ...
  const handleQuickBuyClick = () => {
    const availableFarm = farms.find(
      farm =>
        farm.config?.active &&
        farm.config?.treeCount > 0 &&
        farm.totalSupply > 0
            ...
            <div className="lg:col-span-1 sticky top-[72px] self-start">
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg p-4 sm:p-6 space-y-6 border dark:border-gray-700">
                ...
                <Button
                  ...
                  onClick={() => {
                    setSelectedTrees(1); // or whatever your default/fetched value is
