...
const FarmDetails: React.FC<FarmDetailsProps> = ({ farmId, farmConfig, chainId }) => {
  ...
  // Fetch current TVL from shareTokenAddress (current chain: Scroll or Base)
...
export default function Marketplace() {
  ...
  // Contract reads (current chain: Scroll or Base)
  ...
  // MBT Token balance and allowance (current chain)
  ...
  const approveTokens = async (amount: bigint) => {
    if (!isConnected) {
      ...
      logAction("Approval Failed: Not signed in", { userAddress });
      return false;
    }
    ...
    try {
      ...
      logAction("Initiating MBT Approval", { userAddress, amount: formatUnits(amount, MBT_DECIMALS), contract: mttrVault });
  ...
  const handlePurchase = async () => {
    if (!isConnected) {
      setPurchaseError("Please sign in to continue");
      logAction("Purchase Failed: Not signed in", { userAddress });
      return;
    }
  ...
  // Prefer Openfort when present; otherwise Reown AppKit (email / social / wallet)
  const handleConnectWallet = () => {
    logAction("Sign In Initiated", { userAddress });
    if (typeof (window as unknown as { openfort?: { connect: () => void } }).openfort !== "undefined") {
      ...
      void openSignInModal().catch((err: unknown) => {
        ...
        setPurchaseError("Sign in failed. Please try again.");
      });
      logAction("AppKit Sign In Opened", { userAddress });
    }
  };
  ...
  const setMaxAmount = () => {
    // If user has balance, use the lesser of balance or cap
    // If no balance, show the cap so they know what they need
    const newAmount = userMbtBalance > 0 
      ? Math.min(userMbtBalance, maxInvestmentNum).toFixed(2)
      : maxInvestmentNum.toFixed(2);
    setMbtAmount(newAmount);
    setPurchaseError("");
                ...
                <div className="space-y-6">
                  <div className="p-4 rounded-lg">
                    ...
                    <div className="mt-4">
                      ...
                      >
                        {isConnected ? "Buy Bonds" : "Sign in"}
                      </Button>
                    </div>
                  </div>
                  <FarmDetails farmId={selectedFarmData.farmId.toString()} farmConfig={selectedFarmData.data} chainId={chainId} />
                </div>
              )}
              <DialogFooter>
                <Button
                ...
                {!isConnected ? (
                  <div className="text-center">
                    <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
                      Please sign in to purchase bonds (email, social, or wallet).
                    </p>
                    <Button
                      className="bg-[#7A5540] hover:bg-[#6A4A36] text-white border-none"
