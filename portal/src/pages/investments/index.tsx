...
import { useAccount, useReadContract, useReadContracts, useWriteContract, useBalance, usePublicClient, useChainId } from "wagmi"
...
export default function Investments() {
  ...
  // MBT Token balance (current chain: Scroll or Base)
  ...
  const handleRedeem = async () => {
    if (!isConnected) {
      setRedeemError("Please sign in to continue");
      return;
    }
    ...
    if (!mttrVault) {
      setRedeemError("Contract address not available for current network");
      return;
    }
  ...
  const handleConnectWallet = () => {
    if (typeof (window as unknown as { openfort?: { connect: () => void } }).openfort !== "undefined") {
      ...
      void openSignInModal().catch((err: unknown) => {
        ...
        setRedeemError("Sign in failed. Please try again.");
      });
    }
  };
              ...
              {!isConnected ? (
                <div className="text-center">
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
                    Please sign in to redeem bonds (email, social, or wallet).
                  </p>
                  <Button
